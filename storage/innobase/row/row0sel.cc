	...
	buf_page_t* bpage)		/*!< in: mtr */
{
	...
	if((dict_index_get_space(index) ==llt_space_id) && get_flag(&(bpage->flags), IPLIZED) && !get_flag(&(bpage->flags), NORMALIZE) && page_get_max_trx_id(block->frame)!=0 ){
...
//	bool undo_buffer_miss = row_check_undo_page_buffer_miss(clust_index, rec, offsets);
	...
	if((dict_index_get_space(clust_index) ==llt_space_id) && get_flag(&(bpage->flags), IPLIZED) && !get_flag(&(bpage->flags), NORMALIZE)
