...
static monitor_info_t	innodb_counter_info[] =
{
	...
	/* ========== NVDIMM PPL normalize / de-PPLization counters ========== */
	{"module_nvdimm_ppl", "nvdimm_ppl",
	 "NVDIMM Per-Page Logging Module",
	 MONITOR_MODULE,
	 MONITOR_DEFAULT_START, MONITOR_MODULE_NVDIMM_PPL},

	{"nvdimm_ppl_normalize_total", "nvdimm_ppl",
	 "Total number of PPL normalize/de-PPLize events",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_NORMALIZE_TOTAL},

	{"nvdimm_ppl_normalize_cause1_split", "nvdimm_ppl",
	 "Normalize cause 1: page split / record movement",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_NORMALIZE_CAUSE1},

	{"nvdimm_ppl_normalize_cause2_max_ppl", "nvdimm_ppl",
	 "Normalize cause 2: max PPL size exceeded",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_NORMALIZE_CAUSE2},

	{"nvdimm_ppl_normalize_cause3_area_full", "nvdimm_ppl",
	 "Normalize cause 3: PPL area exhausted",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_NORMALIZE_CAUSE3},

	{"nvdimm_ppl_normalize_cause4_non_ppl", "nvdimm_ppl",
	 "Normalize cause 4: non-PPL target page",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_NORMALIZE_CAUSE4},

	{"nvdimm_ppl_normalize_cause5_pool_init", "nvdimm_ppl",
	 "Normalize cause 5: PPL pool initialization",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_NORMALIZE_CAUSE5},

	{"nvdimm_ppl_normalize_cause6_lsn_gap", "nvdimm_ppl",
	 "Normalize cause 6: LSN gap too large (eager normalize)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_NORMALIZE_CAUSE6},

	{"nvdimm_ppl_log_bytes_total", "nvdimm_ppl",
	 "Total PPL log bytes released on normalize",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LOG_BYTES_TOTAL},

	{"nvdimm_ppl_prebuild_hit", "nvdimm_ppl",
	 "find_prebuilt_page_from_list returned a match (LLT used prebuilt)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_PREBUILD_HIT},

	{"nvdimm_ppl_prebuild_miss", "nvdimm_ppl",
	 "find_prebuilt_page_from_list returned NULL (LLT fell back)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_PREBUILD_MISS},

	{"nvdimm_ppl_undo_stock_calls", "nvdimm_ppl",
	 "row_vers_build_for_consistent_read calls on stock",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_UNDO_STOCK_CALLS},
	{"nvdimm_ppl_undo_stock_len_sum", "nvdimm_ppl",
	 "Sum of undo chain lengths on stock (avg=sum/calls)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_UNDO_STOCK_LEN_SUM},
	{"nvdimm_ppl_undo_stock_len_max", "nvdimm_ppl",
	 "Max undo chain length on stock",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_UNDO_STOCK_LEN_MAX},
	{"nvdimm_ppl_undo_stock_page_miss", "nvdimm_ppl",
	 "Total undo-page buffer misses on stock chain walks",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_UNDO_STOCK_PAGE_MISS},

	{"nvdimm_ppl_undo_warehouse_calls", "nvdimm_ppl",
	 "row_vers_build_for_consistent_read calls on warehouse",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_UNDO_WH_CALLS},
	{"nvdimm_ppl_undo_warehouse_len_sum", "nvdimm_ppl",
	 "Sum of undo chain lengths on warehouse (avg=sum/calls)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_UNDO_WH_LEN_SUM},
	{"nvdimm_ppl_undo_warehouse_len_max", "nvdimm_ppl",
	 "Max undo chain length on warehouse",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_UNDO_WH_LEN_MAX},
	{"nvdimm_ppl_undo_warehouse_page_miss", "nvdimm_ppl",
	 "Total undo-page buffer misses on warehouse chain walks",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_UNDO_WH_PAGE_MISS},

	{"nvdimm_ppl_undo_stock_us_sum", "nvdimm_ppl",
	 "Sum of version-build microseconds on stock (undo path). avg=sum/calls",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_UNDO_STOCK_US_SUM},
	{"nvdimm_ppl_undo_stock_us_max", "nvdimm_ppl",
	 "Max version-build microseconds on stock (undo path)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_UNDO_STOCK_US_MAX},
	{"nvdimm_ppl_undo_warehouse_us_sum", "nvdimm_ppl",
	 "Sum of version-build microseconds on warehouse (undo path). avg=sum/calls",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_UNDO_WH_US_SUM},
	{"nvdimm_ppl_undo_warehouse_us_max", "nvdimm_ppl",
	 "Max version-build microseconds on warehouse (undo path)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_UNDO_WH_US_MAX},
	{"nvdimm_ppl_redo_build_calls", "nvdimm_ppl",
	 "nvdimm_build_prev_vers_with_redo invocations (PPL-MV path)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_REDO_BUILD_CALLS},
	{"nvdimm_ppl_redo_build_us_sum", "nvdimm_ppl",
	 "Sum of version-build microseconds on PPL-MV path. avg=sum/calls",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_REDO_BUILD_US_SUM},
	{"nvdimm_ppl_redo_build_us_max", "nvdimm_ppl",
	 "Max version-build microseconds on PPL-MV path",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_REDO_BUILD_US_MAX},
	{"nvdimm_ppl_redo_out_vers", "nvdimm_ppl",
	 "PPL-MV path returned old_vers != NULL (prebuilt direct used, B1)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_REDO_OUT_VERS},
	{"nvdimm_ppl_redo_out_null", "nvdimm_ppl",
	 "PPL-MV path returned old_vers = NULL (B5: no old version)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_REDO_OUT_NULL},
	{"nvdimm_ppl_redo_out_fail", "nvdimm_ppl",
	 "PPL-MV path returned DB_FAIL (caller falls back to undo)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_REDO_OUT_FAIL},

	{"nvdimm_ppl_path_a_calls", "nvdimm_ppl",
	 "Path A: prebuilt direct success (matched + visible)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_PATH_A_CALLS},
	{"nvdimm_ppl_path_a_us_sum", "nvdimm_ppl",
	 "Path A: total microseconds. avg=sum/calls",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_PATH_A_US_SUM},
	{"nvdimm_ppl_path_a_us_max", "nvdimm_ppl",
	 "Path A: max microseconds",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_PATH_A_US_MAX},
	{"nvdimm_ppl_path_b_fallback", "nvdimm_ppl",
	 "Path B (simplified): prebuilt found but not visible → DB_FAIL → undo",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_PATH_B_FALLBACK},
	{"nvdimm_ppl_path_c_calls", "nvdimm_ppl",
	 "Path C: disk read + PPL forward apply (success)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_PATH_C_CALLS},
	{"nvdimm_ppl_path_c_us_sum", "nvdimm_ppl",
	 "Path C: total microseconds. avg=sum/calls",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_PATH_C_US_SUM},
	{"nvdimm_ppl_path_c_us_max", "nvdimm_ppl",
	 "Path C: max microseconds",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_PATH_C_US_MAX},
	{"nvdimm_ppl_path_c_fail", "nvdimm_ppl",
	 "Path C: disk read + PPL apply attempted but resulted in DB_FAIL",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_PATH_C_FAIL},

	/* LLT-only counters. Mirror the totals above but only count
	   when the calling read view is the LLT view. */
	{"nvdimm_ppl_llt_path_a_calls", "nvdimm_ppl",
	 "LLT only: Path A calls", MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_PATH_A_CALLS},
	{"nvdimm_ppl_llt_path_a_us_sum", "nvdimm_ppl",
	 "LLT only: Path A microseconds sum", MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_PATH_A_US_SUM},
	{"nvdimm_ppl_llt_path_b_fallback", "nvdimm_ppl",
	 "LLT only: Path B fallback events", MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_PATH_B_FALLBACK},
	{"nvdimm_ppl_llt_path_b_us_sum", "nvdimm_ppl",
	 "LLT only: Path B microseconds sum (PPL-MV portion before DB_FAIL)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_PATH_B_US_SUM},
	{"nvdimm_ppl_llt_path_c_calls", "nvdimm_ppl",
	 "LLT only: Path C calls", MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_PATH_C_CALLS},
	{"nvdimm_ppl_llt_path_c_us_sum", "nvdimm_ppl",
	 "LLT only: Path C microseconds sum", MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_PATH_C_US_SUM},
	{"nvdimm_ppl_llt_redo_build_calls", "nvdimm_ppl",
	 "LLT only: total nvdimm_build_prev_vers_with_redo invocations",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_REDO_BUILD_CALLS},
	{"nvdimm_ppl_llt_redo_build_us_sum", "nvdimm_ppl",
	 "LLT only: total redo-build microseconds sum",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_REDO_BUILD_US_SUM},
	{"nvdimm_ppl_llt_prebuild_hit", "nvdimm_ppl",
	 "LLT only: find_prebuilt hit", MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_PREBUILD_HIT},
	{"nvdimm_ppl_llt_prebuild_miss", "nvdimm_ppl",
	 "LLT only: find_prebuilt miss", MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_PREBUILD_MISS},
	{"nvdimm_ppl_llt_undo_stock_calls", "nvdimm_ppl",
	 "LLT only: undo path calls on stock", MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_UNDO_STOCK_CALLS},
	{"nvdimm_ppl_llt_undo_stock_us_sum", "nvdimm_ppl",
	 "LLT only: undo time on stock", MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_UNDO_STOCK_US_SUM},
	{"nvdimm_ppl_llt_undo_stock_len_sum", "nvdimm_ppl",
	 "LLT only: undo chain length sum on stock", MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_UNDO_STOCK_LEN_SUM},
	{"nvdimm_ppl_llt_undo_warehouse_calls", "nvdimm_ppl",
	 "LLT only: undo path calls on warehouse", MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_UNDO_WH_CALLS},
	{"nvdimm_ppl_llt_undo_warehouse_us_sum", "nvdimm_ppl",
	 "LLT only: undo time on warehouse", MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_UNDO_WH_US_SUM},
	{"nvdimm_ppl_llt_undo_warehouse_len_sum", "nvdimm_ppl",
	 "LLT only: undo chain length sum on warehouse",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_UNDO_WH_LEN_SUM},
	{"nvdimm_ppl_llt_undo_district_calls", "nvdimm_ppl",
	 "LLT only: undo path calls on district", MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_UNDO_DISTRICT_CALLS},
	{"nvdimm_ppl_llt_undo_district_us_sum", "nvdimm_ppl",
	 "LLT only: undo time on district", MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_UNDO_DISTRICT_US_SUM},
	{"nvdimm_ppl_llt_undo_district_len_sum", "nvdimm_ppl",
	 "LLT only: undo chain length sum on district",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_UNDO_DISTRICT_LEN_SUM},
	{"nvdimm_ppl_llt_undo_stock_fallback_calls", "nvdimm_ppl",
	 "LLT only: stock undo calls reached via PPL-MV fallback",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_UNDO_STOCK_FB_CALLS},
	{"nvdimm_ppl_llt_undo_stock_fallback_us_sum", "nvdimm_ppl",
	 "LLT only: stock undo µs (fallback subset)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_UNDO_STOCK_FB_US_SUM},
	{"nvdimm_ppl_llt_undo_stock_fallback_len_sum", "nvdimm_ppl",
	 "LLT only: stock undo chain length sum (fallback subset)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_UNDO_STOCK_FB_LEN_SUM},
	{"nvdimm_ppl_llt_find_us_sum", "nvdimm_ppl",
	 "LLT only: time spent in find_prebuilt (excluded from path_*_us_sum)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_FIND_US_SUM},
	{"nvdimm_ppl_llt_find_calls", "nvdimm_ppl",
	 "LLT only: find_prebuilt invocations (= path_a + path_b + path_c)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_FIND_CALLS},
	{"nvdimm_ppl_llt_path2_undo_calls", "nvdimm_ppl",
	 "LLT only: paper [2] internal undo invocations from prebuilt frame",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_PATH2_UNDO_CALLS},
	{"nvdimm_ppl_llt_path2_undo_us_sum", "nvdimm_ppl",
	 "LLT only: paper [2] internal undo time sum",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_PATH2_UNDO_US_SUM},
	{"nvdimm_ppl_llt_path_b_len_sum", "nvdimm_ppl",
	 "LLT only: path B nested undo records walked (chain length sum)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_PATH_B_LEN_SUM},
	{"nvdimm_ppl_llt_path_c_len_sum", "nvdimm_ppl",
	 "LLT only: path C PPL log records forward-applied (chain length sum)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_PATH_C_LEN_SUM},
	{"nvdimm_ppl_llt_path_c_nested_undo_calls", "nvdimm_ppl",
	 "LLT only: path C followed by nested undo (Old Page + Undo)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_PATH_C_NESTED_UNDO_CALLS},
	{"nvdimm_ppl_llt_path_c_nested_undo_us_sum", "nvdimm_ppl",
	 "LLT only: path C nested undo time sum",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_PATH_C_NESTED_UNDO_US_SUM},
	{"nvdimm_ppl_llt_path_c_disk_undo_calls", "nvdimm_ppl",
	 "LLT only: disk-too-new → undo from disk frame (skip forward apply)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_PATH_C_DISK_UNDO_CALLS},
	{"nvdimm_ppl_llt_path_c_disk_undo_us_sum", "nvdimm_ppl",
	 "LLT only: disk-too-new + undo from disk time sum",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_PATH_C_DISK_UNDO_US_SUM},
	{"nvdimm_ppl_llt_path_c_disk_undo_len_sum", "nvdimm_ppl",
	 "LLT only: disk-too-new + undo chain records walked (sum)",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_NVDIMM_PPL_LLT_PATH_C_DISK_UNDO_LEN_SUM},
	{"vanilla_llt_undo_stock_calls", "nvdimm_ppl",
	 "Vanilla LLT (no PPL): stock-table undo build calls",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_VANILLA_LLT_UNDO_STOCK_CALLS},
	{"vanilla_llt_undo_stock_us_sum", "nvdimm_ppl",
	 "Vanilla LLT (no PPL): stock-table undo build time sum",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_VANILLA_LLT_UNDO_STOCK_US_SUM},
	{"vanilla_llt_undo_stock_len_sum", "nvdimm_ppl",
	 "Vanilla LLT (no PPL): stock-table undo chain records walked",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_VANILLA_LLT_UNDO_STOCK_LEN_SUM},
	{"vanilla_llt_undo_warehouse_calls", "nvdimm_ppl",
	 "Vanilla LLT (no PPL): warehouse-table undo build calls",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_VANILLA_LLT_UNDO_WAREHOUSE_CALLS},
	{"vanilla_llt_undo_warehouse_us_sum", "nvdimm_ppl",
	 "Vanilla LLT (no PPL): warehouse-table undo build time sum",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_VANILLA_LLT_UNDO_WAREHOUSE_US_SUM},
	{"vanilla_llt_undo_warehouse_len_sum", "nvdimm_ppl",
	 "Vanilla LLT (no PPL): warehouse-table undo chain records walked",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_VANILLA_LLT_UNDO_WAREHOUSE_LEN_SUM},
	{"vanilla_llt_undo_district_calls", "nvdimm_ppl",
	 "Vanilla LLT (no PPL): district-table undo build calls",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_VANILLA_LLT_UNDO_DISTRICT_CALLS},
	{"vanilla_llt_undo_district_us_sum", "nvdimm_ppl",
	 "Vanilla LLT (no PPL): district-table undo build time sum",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_VANILLA_LLT_UNDO_DISTRICT_US_SUM},
	{"vanilla_llt_undo_district_len_sum", "nvdimm_ppl",
	 "Vanilla LLT (no PPL): district-table undo chain records walked",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_VANILLA_LLT_UNDO_DISTRICT_LEN_SUM},
	{"undo_write_stock", "nvdimm_ppl",
	 "Per-table undo write count: stock",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_UNDO_WRITE_STOCK},
	{"undo_write_warehouse", "nvdimm_ppl",
	 "Per-table undo write count: warehouse",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_UNDO_WRITE_WAREHOUSE},
	{"undo_write_district", "nvdimm_ppl",
	 "Per-table undo write count: district",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_UNDO_WRITE_DISTRICT},
	{"undo_write_customer", "nvdimm_ppl",
	 "Per-table undo write count: customer",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_UNDO_WRITE_CUSTOMER},
	{"undo_write_orderline", "nvdimm_ppl",
	 "Per-table undo write count: order_line",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_UNDO_WRITE_ORDERLINE},
	{"undo_write_order", "nvdimm_ppl",
	 "Per-table undo write count: orders",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_UNDO_WRITE_ORDER},
	{"undo_write_neworder", "nvdimm_ppl",
	 "Per-table undo write count: new_orders",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_UNDO_WRITE_NEWORDER},
	{"undo_write_history", "nvdimm_ppl",
	 "Per-table undo write count: history",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
	 MONITOR_UNDO_WRITE_HISTORY},
	{"undo_write_other", "nvdimm_ppl",
	 "Per-table undo write count: other tables",
	 MONITOR_DEFAULT_ON, MONITOR_DEFAULT_START,
